...
const ChatWidgetUI: React.FC<Props> = ({
  ...
  const sendUserMessage = useCallback(
    (message: string, options?: { triggeredByVoice?: boolean }) => {
      if (!message.trim()) {
        return;
      }
  ...
  const finalizeSendModeMessage = useCallback(() => {
    ...
    if (message) {
      sendUserMessage(message, { triggeredByVoice: true });
    }
  ...
  const handleSendWithDictation = useCallback(
    (messageOverride?: string, options?: { triggeredByVoice?: boolean }) => {
      ...
      clearDictationSession();
      sendUserMessage(messageToSend, { triggeredByVoice });
      setInput("");
    },
    [
      clearDictationSession,
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      recognitionRef.current.onresult = (event: any) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; i += 1) {
          ...
          if (result.isFinal || mode === "dictate") {
            collectedTranscript += result[0].transcript;
          }
        }

        const cleanedTranscript = collectedTranscript
          .replace(/\s+/g, " ")
          .trim();
        ...
        if (mode === "dictate") {
          ...
          if (triggered) {
            ...
            handleSendWithDictation(finalContent, { triggeredByVoice: true });
          }
        }
      };
              ...
              <div className='flex items-center gap-1'>
                ...
                >
                  ...
                  <svg
                    ...
                    fill={isDictateActive ? "currentColor" : "none"}
                    viewBox='0 0 24 24'
                    stroke='currentColor'
                    strokeWidth={2}
                  >
